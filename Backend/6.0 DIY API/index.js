...
app.get("/jokes/:type", (req, res) => {
  ...
  const filteredJones = jokes.filter((j) => j.jokeType.toLowerCase() === jokeType.toLowerCase());
  if (filteredJones.length > 0) {
    res.json(filteredJones);
  } else {
...
app.delete("/jokes", (req, res) => {
  ...
  if (key === masterKey) {
    ...
    res.status(403).json({ error: "Invalid master key. Access denied." });
  }
});
